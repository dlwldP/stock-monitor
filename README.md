...
- 테스트: `./gradlew test` — 112개. 컨트롤러 HTTP 계약 테스트(`@WebMvcTest`) 포함. 도메인 로직(알림 조건 판정·쿨다운)·서비스·스케줄러·알림 디스패처 단위 테스트(Mockito 기반), 토스 API 응답 매핑을 실제 응답 구조에 고정하는 테스트, 캔들 기반 등락률/거래량/52주 파생 계산 테스트
...
- 시세는 **여러 종목을 한 번에** 조회합니다 (`symbols=A,B,C`). 파라미터가 복수형이지만 문서에 다중 조회 지원 여부가 없어서, 런타임에 확인합니다 — 여러 종목을 요청했는데 결과가 1개만 오면 그 프로세스에서는 이후 종목별 개별 호출로 되돌립니다 (`TossHttpApiClient.getQuotes`). 응답에서 일부 종목이 빠지면 그 종목만 개별 재조회합니다
...
`GET /api/v1/prices`가 마지막 체결가만 돌려주기 때문에, 등락률·거래량·52주 고저를 직접 주는 엔드포인트는 없습니다. 대신 이미 동작 확인된 캔들 API(`GET /api/v1/candles`)의 일봉 데이터(고가/저가/거래량/종가)로 **파생 계산**합니다 (`TossHttpApiClient.getQuote` → `computeEnrichment`):

| 조건 | Mock | 실연동 |
|---|---|---|
| 목표가 이상/이하 (PRICE_ABOVE/BELOW) | ✅ | ✅ (원본 데이터) |
| 등락률 (PCT_CHANGE) | ✅ | ✅ (파생) — 마지막 체결가 vs. 직전 거래일 종가 |
| 거래량 급증 (VOLUME_SPIKE) | ✅ | ✅ (파생) — 가장 최근 캔들 거래량 vs. 그 이전 20일 평균 |
| 52주 신고가/신저가 근접 (WEEK52_*) | ✅ | ✅ (파생) — 캔들 히스토리 전체(최대 400일 요청) 고저 + 현재가 포함 |

**파생 계산의 한계** (README·코드 Javadoc 어디든 과장하지 않으려 명시):

- 캔들 API는 `count` 파라미터가 미확정이라 몇 거래일치를 돌려주는지 API가 정하는 대로입니다. "52주"라고 표기하지만 실제로는 "API가 기본으로 주는 히스토리만큼"이라 정확히 252거래일을 보장하지 않습니다. 특정 종목이 얼마나 돌려주는지는 `GET /api/toss/raw/candles?symbol=...&days=1000`으로 직접 확인할 수 있습니다.
- 거래량은 실시간 체결량이 아니라 **가장 최근에 확정된 일봉의 거래량**입니다. 장중 실시간 거래량 급증 감지가 아니라 "최근 일봉 vs 과거 평균" 비교에 가깝습니다.
- 캔들 히스토리는 심볼당 30분 캐싱합니다 (하루 단위로 바뀌는 데이터라 매 폴링(60초)마다 다시 받아올 필요가 없어서). 캔들 조회가 실패해도 시세 조회 자체는 계속 동작하고, 파생 필드만 비어있는 상태로 반환됩니다 (`AlertRule.isSatisfiedBy`가 누락된 입력은 그냥 미충족으로 처리 — 규칙 하나 때문에 폴링 전체가 죽지 않도록).

더 정확한 값을 주는 전용 엔드포인트(등락률/거래량/52주)를 문서에서 찾으면, `TossHttpApiClient.getQuote`의 파생 로직을 그 엔드포인트로 교체하면 됩니다.
...
- **토스증권 연동**: 실계좌 연동 동작 확인 완료 (위 "토스증권 실연동" 절 참고). 기본값은 여전히 `MockTossApiClient`(랜덤워크 가상 시세, 52주 고저·평균거래량·60일 캔들 포함)이고, `TOSS_API_USE_REAL_CLIENT=true`로 켜면 실 API를 씁니다. 알림 조건 6개 전부 두 모드에서 동작하지만, 실연동의 등락률/거래량/52주 값은 API가 직접 주는 게 아니라 캔들 데이터로 파생 계산한 값입니다 ("실연동 시 제약" 참고).
- **관심종목**: 추가/삭제/목록 조회 (`/api/watchlist`), 실시간 시세·등락률 포함 (실연동은 캔들 기반 파생값)
- **알림 규칙**: 목표가 이상/이하, 등락률(±N%), 거래량 급증(평균 대비 N배), 52주 신고가/신저가 근접 — 6가지 조건 × 디스코드/이메일/인앱 채널, 쿨다운, **반복 방식**, 활성/비활성 토글, 생성/삭제 (`/api/alert-rules`). **임계값·채널·쿨다운·반복 방식은 만든 뒤에도 수정 가능**합니다(`PATCH /api/alert-rules/{id}`) — 종목·조건 자체를 바꾸려면 여전히 삭제 후 재생성입니다(다른 규칙으로 취급)
- **알림 발송 재시도**: 디스코드/이메일 발송이 네트워크 문제 등으로 실패하면 1회 자동 재시도합니다 (`NotificationDispatcher`). 웹훅 URL 미설정처럼 재시도해도 똑같이 실패할 게 뻔한 설정 오류는 재시도하지 않고 바로 실패 기록만 남깁니다 — 오류 원인에 원인 예외(cause)가 붙어 있는지로 구분합니다
  - **반복 방식**(`triggerMode`)은 조건이 계속 충족된 상태로 유지될 때의 동작을 정합니다. 쿨다운만으로는 표현이 안 되는 부분입니다 — 목표가를 넘긴 채 주가가 머물면 매 폴링마다 조건이 충족되므로, 쿨다운이 유일한 방어막이면 그 가격이 유지되는 내내 쿨다운 주기마다 계속 알림이 옵니다.
    - `EDGE`(기본값): 조건이 **충족되는 순간 1회**만 알리고, 조건이 해제됐다가 다시 충족될 때 재발동. 쿨다운도 함께 적용돼서 임계값 근처를 오르내려도 알림이 몰아치지 않습니다.
    - `REPEAT`: 조건이 유지되는 동안 쿨다운마다 반복 (기존 동작)
...
- **데이터 보관 정책**: 타이머로 계속 쌓이는 두 테이블을 매일 03:30(설정 가능)에 정리합니다 (`RetentionService`). 알림 로그는 90일이 지나면 삭제하고, 자산 스냅샷은 **삭제하지 않고 30일이 지난 것부터 하루 1건으로 솎아냅니다** — 1년 전 자산 가치는 자산 추이 차트에서 가장 볼 만한 정보라 지우면 안 되지만, 15분 간격 원본을 영구 보관할 이유도 없어서입니다 (1년 기준 약 35,000행 → 약 365행). `retention.*`로 조정하고, `0` 이하로 두면 해당 정리를 끄는 것이지 전부 삭제하는 게 아닙니다
