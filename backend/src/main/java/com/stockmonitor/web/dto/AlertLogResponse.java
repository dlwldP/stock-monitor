...
public record AlertLogResponse(
		Long id, Long alertRuleId, String symbol, Instant triggeredAt, AlertChannel channel, AlertLogStatus status, String message) {
	...
	public static AlertLogResponse of(AlertLog logEntry) {
		return new AlertLogResponse(
				...
				logEntry.getTriggeredAt(), logEntry.getChannel(), logEntry.getStatus(), logEntry.getMessage());
	}
}
